...
function rebuildView(refit=true){
  ...
  const nodes=sourceNodes.filter(n=>scopeIncludes(n.id));
  const ids=new Set(nodes.map(n=>n.id)),edges=sourceEdges.filter(e=>ids.has(e[0])&&ids.has(e[1]));
  const projected=AtlasGraph.collapse(nodes,edges,[...collapsedGroups]);
  ...
  const positioned=layoutMode==='auto'?AtlasGraph.layout(projected.nodes,projected.edges):projected.nodes;
  N.splice(0,N.length,...positioned);E.splice(0,E.length,...AtlasGraph.route(positioned,projected.edges));
...
function reviewNodeHTML(n){
  const status=componentChange(n);if(!status)return '';
  const paths=new Set(review.nodes[n.id]?.files||[]),files=review.files.filter(f=>paths.has(f.path)||paths.has(f.oldPath));
  return `<section class="review-summary"><span class="change-chip ${esc(status)}">${esc(status)}</span>
    <p>${status==='removed'?'Historical component from the baseline atlas.':'Changed files mapped to this component. Review its evidence against the new code.'}</p>${reviewFilesHTML(files)}</section>`;
}
...
function initExplorer(){
  ...
  document.getElementById('review-scope')?.addEventListener('change',e=>{resetExplorer();reviewScope=e.target.value;rebuildView();render();});
  document.getElementById('review-context')?.addEventListener('change',e=>{resetExplorer();showContext=e.target.checked;rebuildView();render();});
